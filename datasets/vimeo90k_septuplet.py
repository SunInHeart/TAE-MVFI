...
class VimeoSepTuplet(Dataset):
    def __init__(self, data_root, is_training, input_frames="17", mode='full', finetune=False):  # original mode='mini'
        """
    ...
    def __getitem__(self, index):
        ...
        # inputs = [int(e)-1 for e in list(self.inputs)]  # [0, 2, 4, 6]
        # inputs = inputs[:len(inputs)//2] + [3] + inputs[len(inputs)//2:]  # [0, 2, 3, 4, 6]
        # images = [images[i] for i in inputs]  # [im1, im3, im4, im5, im7]
        ...
        if self.training:
            ...
            if random.random() >= 0.5:
                images = images[::-1]
                imgpaths = imgpaths[::-1]
            gts = images[1:len(images) - 1]  # [im2, im3, im4, im5, im6]
            images = [images[0], images[len(images) - 1]]  # [im1, im7]
...
def get_loader(mode, data_root, batch_size, shuffle, num_workers, test_mode=None, finetune=False):
    if mode == 'train':
        ...
        is_training = False
    dataset = VimeoSepTuplet(data_root, is_training=is_training, finetune=finetune)
